...
- [x] In some apps, if I select a section of text (eg: in Google Chrome full URL in navigation bar) when I open Funes that selection is lost. So when pasting it's actually appended to the url instead of replaced. Also happens in nemo, doesn't happen in Intellij. In xed it prepends. — the popup was a normal focusable window, so mapping it made the WM focus Funes and sent the source window a real `FocusOut` (mode `NotifyNormal`); Chrome's omnibox, nemo and xed clear their selection on that (IntelliJ/Java keeps it, hence the inconsistency). Fixed with the rofi/CopyQ model: the popup now refuses focus (`set_accept_focus(False)` → `WM_HINTS.input = False`, no `present()`) and grabs the seat instead (`app/grab.py::SeatGrab`), which only produces a `NotifyGrab` focus change — toolkits ignore those, so the selection and `_NET_ACTIVE_WINDOW` stay put. GTK is kept usable by a synthesized toplevel focus-in (`PopupWindow._fake_toplevel_focus`), and closing is driven by the grab (click outside the frame, `grab-broken-event`) instead of deactivation. If the grab cannot be taken the popup is not shown and a notification explains why. X11 only, see [`WAYLAND.md`](WAYLAND.md).
 
  Second root cause, xed only: `paste-sets-primary` (default on) made Funes take the PRIMARY selection on every paste, and the resulting `SelectionClear` makes GtkSourceView/GtkTextView drop its selection - so the selection survived opening the popup but died at paste time, and the text landed at the caret. PRIMARY is only needed by terminals whose Shift+Insert reads it (xterm, urxvt), so the boolean was replaced by `paste-primary-class-regex` (default `xterm|urxvt|rxvt`, matched against the target's `WM_CLASS` like `paste-ctrl-v-class-regex`; empty never touches PRIMARY) - `data/org.x.funes.gschema.xml`, `funes/paster.py::class_matches`, `app/funes_app.py`, `app/preferences.py`. Third root cause, terminals: VTE also reads Shift+Insert from PRIMARY, so narrowing PRIMARY broke pasting in xfce4-terminal/Terminator/Tilix. The keystroke is now chosen per target window (Diodon's model) instead of being one global default: Ctrl+V everywhere, Ctrl+Shift+V in terminals (VTE, kitty, alacritty, wezterm, foot, Konsole, Yakuake), Shift+Insert + PRIMARY only for xterm/urxvt/rxvt, which have no Ctrl+Shift+V. Table + matching live in the GTK-free `funes/paste_keys.py` (`tests/test_paste_keys.py`); `paste-ctrl-v-class-regex` and `paste-primary-class-regex` are replaced by a single `paste-ctrl-shift-v-class-regex`, whose default *is* the terminal list (visible and editable in Settings instead of hidden in code); only the xterm/urxvt PRIMARY rule stays built in.
...
- [x] Image support
- [ ] Rich text support.
- [x] File copy paste support: copying or cutting a file in any file manager and then pasting should work as if no clipboard manager was installed (all types supported should still be supported). Cut and copy operation should be registered in funes showing the file name, where the file was copied from and a distinctive icon. — `text/uri-list` + `x-special/gnome-copied-files` supported (KDE's `application/x-kde-cutselection` deliberately deferred, see [`CLIPBOARD.md`](CLIPBOARD.md)); dedicated `files` kind with a cut/copy icon + filename row, and paste replays the original clipboard bytes verbatim (real paste-as-files, not text). "Where the file was copied from" (source app) is intentionally not included yet — depends on the separate WM_CLASS-capture Ledger item below.
- [x] Copy pasting from software like inkscape should work correctly, currently if I copy svg data it gets pasted as an image. — root cause was `Gtk.Clipboard.set_with_data()` being unusable from PyGObject (`_unsupported_data_method`, always raises `AttributeError`), so both the reown step and paste-from-popup silently fell back to a single rasterized PNG every time. Fixed by replacing it with `Gtk.selection_add_target()`/`Gtk.selection_owner_set()` + selection signals on a `Gtk.Invisible` widget (`app/clipboard.py::_own_clipboard_verbatim`), regression-tested in `tests/test_clipboard.py`. Storing/pasting the SVG rep itself never decodes it to a pixbuf (thumbnail-only rsvg use, canonical-mime priority given to the vector rep over any co-offered PNG preview) — see CLIPBOARD.md.
- [ ] Make password copying configurable, also configure whether to show the password as plain text or masked.
- [x] **Foundational** — generalize clipboard capture to be format-agnostic (capture every offered target verbatim, not just `image/*` + a fixed text-atom allowlist) so no current or future clipboard format is silently dropped; `kind` becomes a mime-driven display hint with a guaranteed `other`/opaque fallback instead of a capture-time branch. Design + rollout plan: [`CLIPBOARD.md`](CLIPBOARD.md).
- [x] Treat `image/svg+xml` / `image/x-inkscape-svg` as vector data end-to-end: thumbnail via rsvg, but never decode-then-reencode the stored/pasted bytes (depends on the foundational item above).
- [x] Generic "other" row presentation for unrecognized mimes (label + byte size), so unsupported-today formats still round-trip instead of vanishing (depends on the foundational item above).
